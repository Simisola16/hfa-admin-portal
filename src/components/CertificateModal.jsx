  ...
  const generateLivePreview = useCallback(async (silent = false) => {
    ...
    try {
      ...
      const res = await api.post('/api/certificates/preview-live', {
        ...
        company_name: (certificateForm.company_name || app?.establishment_name || '').trim(),
        company_address: (certificateForm.company_address || app?.establishment_address || '').trim(),
        manufacturing_address: (certificateForm.manufacturing_address || '').trim(),
        scope: (certificateForm.scope || app?.scope || '').trim(),
        product_category: (certificateForm.scope || app?.category || '').trim(),
