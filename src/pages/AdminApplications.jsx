                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                          ...
                          <div>
                            <div style={{ fontWeight: 800, fontSize: 14, color: '#166534', marginBottom: 3 }}>
                              {existingInvoice ? '✓ Invoice Already Sent' : 'Proposal Accepted — Upload Invoice'}
                            </div>
                            <div style={{ fontSize: 12, color: '#15803d', lineHeight: 1.4 }}>
                              {existingInvoice
                                ? `Invoice ${existingInvoice.invoice_number} sent · £${existingInvoice.amount} · Status: ${existingInvoice.status}`
                                : 'The client accepted your proposal. Upload and send the invoice to proceed.'}
                            </div>
                          </div>
                        </div>
                        <button
                          ...
                          onClick={() => {
                            setInvoiceForm({ title: `Invoice for ${manageModal.application_number}`, amount: existingProposal.estimated_cost || '', due_date: '', notes: '', file: null });
                            setShowInvoiceModal(true);
                          }}
                        >
                          {existingInvoice ? '↑ Resend Invoice' : '↑ Upload Invoice'}
                        </button>
                      </div>
                    )}
      ...
      {showInvoiceModal && manageModal && (
        <div className="modal-overlay" style={{ zIndex: 1100 }}>
          <div className="modal" style={{ maxWidth: 560 }} onClick={e => e.stopPropagation()}>
            <div className="modal-header" style={{ background: 'linear-gradient(135deg, #f0fdf4, #fff)', borderBottom: '2px solid #86efac' }}>
              <div>
                ...
                <div style={{ fontSize: 12, color: '#15803d', marginTop: 4, fontWeight: 600 }}>
                  {manageModal.profiles?.company_name} · {manageModal.application_number}
                </div>
              </div>
            ...
            <div className="modal-body" style={{ maxHeight: '70vh', overflowY: 'auto' }}>
              <p style={{ fontSize: 13, color: '#64748b', marginBottom: 20 }}>
                Upload the invoice for <strong>{manageModal.profiles?.company_name}</strong>. The client will be notified and can confirm payment from their portal.
              </p>
              ...
              <div className="form-group">
                ...
                <textarea
                  ...
                  placeholder="e.g. Payment due within 14 days. Bank transfer details on the invoice."
                />
              </div>
            </div>
            <div className="modal-footer" style={{ background: '#f8fafc', borderTop: '1px solid #e2e8f0' }}>
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    const clientId = manageModal.client_id || manageModal.profiles?._id || manageModal.profiles?.id;
                    if (!clientId) { toast.error('Cannot identify client ID'); setInvoiceSubmitting(false); return; }
                    const appId = manageModal._id || manageModal.id;
                    formData.append('application_id', appId);
                    formData.append('client_id', clientId);
                    ...
                    toast.success('🧾 Invoice uploaded and sent to client!');
              ...
              >
                {invoiceSubmitting ? 'Uploading...' : '🧾 Send Invoice to Client'}
              </button>
            </div>
          </div>
        </div>
