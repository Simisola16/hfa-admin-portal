...
export default function AdminAudits() {
  ...
  const handleCloseNc = async (auditId, appId, reportId) => {
    ...
    try {
      ...
      fetchAudits();
      // Update the local open modal state
  ...
  return (
    <div className="page-content">
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24, flexWrap: 'wrap', gap: 16 }}>
        <div>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: 13.5, margin: 0 }}>
            Track audit schedules, assigned auditor teams, site locations, and manage Non-Conformities (NCs) &amp; client evidence.
          </p>
        </div>
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          ...
          >
            <FileText size={15} /> Audit Reports Repository
          </button>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(210px, 1fr))', gap: 16, marginBottom: 24 }}>
        <div
          className="card"
          style={{ padding: '18px 20px', borderRadius: 16, cursor: 'pointer', border: activeTab === 'all' ? '2px solid var(--primary)' : '1px solid #e2e8f0' }}
          onClick={() => setSearchParams({ filter: 'all' })}
        >
          <div style={{ fontSize: 11.5, fontWeight: 700, color: '#64748b', textTransform: 'uppercase' }}>Total Audits</div>
          <div style={{ fontSize: 24, fontWeight: 800, color: 'var(--primary)', marginTop: 4 }}>{metrics.total}</div>
          <div style={{ fontSize: 12, color: '#94a3b8', marginTop: 2 }}>Across all facilities</div>
        </div>

        <div
          className="card"
          style={{ padding: '18px 20px', borderRadius: 16, cursor: 'pointer', border: activeTab === 'upcoming' ? '2px solid #0284c7' : '1px solid #e2e8f0' }}
          onClick={() => setSearchParams({ filter: 'upcoming' })}
        >
          <div style={{ fontSize: 11.5, fontWeight: 700, color: '#0369a1', textTransform: 'uppercase' }}>Upcoming / Scheduled</div>
          <div style={{ fontSize: 24, fontWeight: 800, color: '#0284c7', marginTop: 4 }}>{metrics.upcoming}</div>
          <div style={{ fontSize: 12, color: '#0284c7', marginTop: 2 }}>{metrics.pending} dates pending confirmation</div>
        </div>
        ...
        <div
          ...
          style={{
            ...
            background: metrics.activeNc > 0 ? 'linear-gradient(135deg, #fff, #fef2f2)' : 'white'
          }}
          onClick={() => setSearchParams({ filter: 'ncs' })}
        >
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ fontSize: 11.5, fontWeight: 700, color: '#b91c1c', textTransform: 'uppercase' }}>Non-Conformities (NCs)</div>
            {metrics.activeNc > 0 && (
              <span style={{ fontSize: 10, fontWeight: 800, background: '#fee2e2', color: '#b91c1c', padding: '2px 6px', borderRadius: 4 }}>
                ACTION NEEDED
              </span>
            )}
          </div>
          <div style={{ fontSize: 24, fontWeight: 800, color: '#dc2626', marginTop: 4 }}>{metrics.activeNc}</div>
          <div style={{ fontSize: 12, color: '#991b1b', marginTop: 2 }}>{metrics.withNc} total NC records</div>
        </div>

        <div
          className="card"
          style={{ padding: '18px 20px', borderRadius: 16, cursor: 'pointer', border: activeTab === 'completed' ? '2px solid #16a34a' : '1px solid #e2e8f0' }}
          onClick={() => setSearchParams({ filter: 'completed' })}
        >
          <div style={{ fontSize: 11.5, fontWeight: 700, color: '#15803d', textTransform: 'uppercase' }}>Audit Completed</div>
          <div style={{ fontSize: 24, fontWeight: 800, color: '#16a34a', marginTop: 4 }}>{metrics.completed}</div>
      ...
      {/* Search Toolbar */}
      <div className="card" style={{ padding: '14px 18px', borderRadius: 14, marginBottom: 18 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 14, flexWrap: 'wrap' }}>
          <div style={{ position: 'relative', flex: 1, minWidth: 260 }}>
            <Search size={16} style={{ position: 'absolute', left: 14, top: '50%', transform: 'translateY(-50%)', color: '#94a3b8' }} />
            <input
              type="text"
              placeholder="Search by company name, site location, application ref, auditor name, or NC observation..."
              value={search}
              onChange={e => setSearch(e.target.value)}
              className="form-control"
              style={{ paddingLeft: 40, height: 40, borderRadius: 8, fontSize: 13.5 }}
            />
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <select
              ...
              style={{ height: 40, borderRadius: 8, fontSize: 13, width: 140 }}
            >
        ...
        <div className="table-wrap">
          {loading ? (
            <div style={{ padding: 60, textAlign: 'center' }}>
              ...
              <div style={{ color: 'var(--text-muted)', fontSize: 13.5 }}>Loading audit records...</div>
            </div>
            ...
            <table>
              <thead>
                <tr>
                  <th style={{ padding: '14px 20px' }}>Company &amp; Site</th>
                  <th style={{ padding: '14px 20px' }}>Type &amp; Stage</th>
                  <th style={{ padding: '14px 20px' }}>Assigned Auditor Team</th>
                  <th style={{ padding: '14px 20px' }}>Audit Schedule</th>
                  <th style={{ padding: '14px 20px' }}>Audit Status</th>
                  <th style={{ padding: '14px 20px' }}>Non-Conformity (NC)</th>
              ...
              <tbody>
                {filteredAudits.map(a => {
                  ...
                  const appRef = a.applications?.application_number || a.application_id?.application_number || 'HFA Audit';
                  const compName = a.company_name || a.profiles?.company_name || a.applications?.establishment_name || 'Client Facility';
                  const siteName = a.site_name || a.sites?.name || a.applications?.site_name || 'Main Site';
                  const ncSummary = getAuditNcSummary(a);
                  const auditDate = a.finalized_date || a.scheduled_date || a.selected_dates?.[0];

                  return (
                    <tr key={auditId} style={{ borderBottom: '1px solid #f1f5f9' }}>
                      {/* Company & Site */}
                      <td style={{ padding: '16px 20px' }}>
                        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                          <div style={{ width: 34, height: 34, borderRadius: 8, background: '#f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: 2 }}>
                            <Building2 size={16} style={{ color: '#475569' }} />
                          </div>
                          <div>
                            <div style={{ fontWeight: 800, fontSize: 13.5, color: '#0f172a' }}>{compName}</div>
                            ...
                            <div style={{ marginTop: 4 }}>
                              <span style={{ fontSize: 10.5, fontWeight: 700, background: '#e0f2fe', color: '#0369a1', padding: '2px 6px', borderRadius: 4 }}>
                                Ref: {appRef}
                              </span>
                            </div>
                          </div>
                      ...
                      <td style={{ padding: '16px 20px' }}>
                        ...
                        {a.stage && (
                          <div style={{ marginTop: 4 }}>
                            <span style={{ fontSize: 11, fontWeight: 800, padding: '2px 8px', borderRadius: 12, background: a.stage === 2 ? '#fdf4ff' : '#f0fdf4', color: a.stage === 2 ? '#a21caf' : '#15803d', border: `1px solid ${a.stage === 2 ? '#f0abfc' : '#bbf7d0'}` }}>
                              Stage {a.stage}
                            </span>
                          </div>
                        )}
                        ...
                        {a.auditors && a.auditors.length > 0 ? (
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                            {a.auditors.map((aud, i) => (
                              <div key={i} style={{ fontSize: 12, color: '#1e293b', display: 'flex', alignItems: 'center', gap: 6 }}>
                                <UserCheck size={13} style={{ color: '#0284c7' }} />
                                <span style={{ fontWeight: 700 }}>{aud.name}</span>
                                {aud.role && (
                          ...
                          <div style={{ fontSize: 12, color: '#94a3b8', fontStyle: 'italic' }}>
                            {a.inspectors?.full_name || 'No auditors assigned'}
                          </div>
                        )}
                      </td>
                        ...
                        {auditDate ? (
                          <div>
                            <div style={{ fontSize: 13, fontWeight: 700, color: '#0f172a', display: 'flex', alignItems: 'center', gap: 5 }}>
                              <Calendar size={13} style={{ color: '#15803d' }} />
                          ...
                          <div>
                            <span style={{ fontSize: 11.5, fontWeight: 700, color: '#b45309', background: '#fffbeb', padding: '3px 8px', borderRadius: 6, border: '1px solid #fde68a' }}>
                              3 Dates Proposed
                            </span>
                            <div style={{ fontSize: 11, color: '#94a3b8', marginTop: 4 }}>Awaiting client choice</div>
                          </div>
                        ) : a.status === 'dates_accepted' ? (
                          <div>
                            <span style={{ fontSize: 11.5, fontWeight: 700, color: '#0369a1', background: '#eff6ff', padding: '3px 8px', borderRadius: 6, border: '1px solid #bfdbfe' }}>
                              Dates Accepted
                            </span>
                            <div style={{ fontSize: 11, color: '#0284c7', marginTop: 4 }}>Ready for final date &amp; team</div>
                          </div>
                        ) : (
                          <span style={{ fontSize: 12, color: '#94a3b8' }}>Unscheduled</span>
                        )}
                      </td>

                      {/* Audit Status */}
                      <td style={{ padding: '16px 20px' }}>
                        <span className={`badge ${STATUS_BADGE[a.status] || 'badge-gray'}`} style={{ fontWeight: 800 }}>
                      ...
                      <td style={{ padding: '16px 20px' }}>
                        {ncSummary.count === 0 ? (
                          <span style={{ fontSize: 11.5, fontWeight: 700, color: '#15803d', background: '#f0fdf4', border: '1px solid #bbf7d0', padding: '3px 8px', borderRadius: 6, display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                            <CheckCircle2 size={12} /> Clean / No NC
                          </span>
                          ...
                          <span style={{ fontSize: 11.5, fontWeight: 800, color: '#7c3aed', background: '#f5f3ff', border: '1px solid #ddd6fe', padding: '3px 8px', borderRadius: 6, display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                            <FileText size={12} /> 📋 Client Evidence Uploaded
                          </span>
                        ) : ncSummary.hasActive ? (
                          <span style={{ fontSize: 11.5, fontWeight: 800, color: '#b91c1c', background: '#fef2f2', border: '1px solid #fecaca', padding: '3px 8px', borderRadius: 6, display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                            <AlertTriangle size={12} /> ⚠️ {ncSummary.count} NC Flagged
                          </span>
                        ) : (
                          <span style={{ fontSize: 11.5, fontWeight: 700, color: '#15803d', background: '#f0fdf4', border: '1px solid #bbf7d0', padding: '3px 8px', borderRadius: 6, display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                      ...
                      <td style={{ padding: '16px 20px', textAlign: 'right' }}>
                        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
                          ...
                          <button
                            ...
                            style={{ fontSize: 12, fontWeight: 700, padding: '6px 10px', display: 'inline-flex', alignItems: 'center', gap: 4 }}
          ...
          <div className="modal" style={{ maxWidth: 680, width: '94%' }} onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div>
                  <div className="modal-title">Non-Conformity (NC) &amp; Findings Management</div>
            ...
            <div className="modal-body" style={{ maxHeight: '72vh', overflowY: 'auto' }}>
                      ...
                      >
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                          <div style={{ fontSize: 13, fontWeight: 800, color: isClosed ? '#15803d' : '#b91c1c', display: 'flex', alignItems: 'center', gap: 6 }}>
                            {isClosed ? <CheckCircle size={16} /> : <AlertTriangle size={16} />}
                            NC Finding #{idx + 1}
                          </div>
                          <span
                            style={{
                        ...
                        {/* Observation Text */}
                        <div style={{ background: '#fafafb', border: '1px solid #e2e8f0', borderRadius: 8, padding: '12px 14px', marginBottom: 12 }}>
                          <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', color: '#64748b', marginBottom: 4 }}>
                            Auditor Observation / Finding:
                          </div>
                          <div style={{ fontSize: 13.5, color: '#0f172a', lineHeight: 1.5 }}>
                        ...
                        {hasClientResp && (
                          <div style={{ background: '#f5f3ff', border: '1px solid #ddd6fe', borderRadius: 10, padding: 14, marginTop: 12, marginBottom: 12 }}>
                            <div style={{ fontSize: 11.5, fontWeight: 800, color: '#6d28d9', textTransform: 'uppercase', marginBottom: 6, display: 'flex', alignItems: 'center', gap: 6 }}>
                              <FileText size={14} /> Client Corrective Action Response
                            </div>
                            {nc.client_response && (
                              <div style={{ fontSize: 13, color: '#4c1d95', marginBottom: 8, lineHeight: 1.5 }}>
                            ...
                            {(nc.correction_document_url || nc.client_response_url) && (
                              <a
                                ...
                                rel="noreferrer"
                                className="btn btn-primary btn-sm"
                                style={{ background: '#6d28d9', borderColor: '#6d28d9', fontSize: 12, display: 'inline-flex', alignItems: 'center', gap: 6 }}
                              >
                        ...
                        {nc.admin_reply && (
                          <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 8, padding: 12, marginTop: 8 }}>
                            ...
                            <div style={{ fontSize: 13, color: '#166534' }}>{nc.admin_reply}</div>
                          </div>
                        )}

                        {/* NC Action Footer */}
                        {!isClosed && (
                          <div style={{ marginTop: 14, paddingTop: 12, borderTop: '1px solid #e2e8f0', display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
                            <button
                              className="btn btn-primary btn-sm"
                              style={{ background: '#16a34a', borderColor: '#16a34a', fontWeight: 700, gap: 6 }}
                            ...
                            >
                              <CheckCircle size={14} /> Close &amp; Accept Non-Conformity
                            </button>
                          </div>
                        )}
                      </div>
              ...
              {selectedAuditForNc.nc_reports && selectedAuditForNc.nc_reports.some(r => r.status !== 'closed') && (
                <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 12, padding: 16, marginBottom: 20 }}>
                  <div style={{ fontSize: 12.5, fontWeight: 800, color: '#1e293b', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 6 }}>
                    <MessageSquare size={14} style={{ color: 'var(--primary)' }} /> Send Additional Guidance / Reply to Client
                  </div>
                  <textarea
                    className="form-control"
                    rows={3}
                    ...
                    style={{ fontSize: 13, background: 'white', marginBottom: 10 }}
                  />
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: 10 }}>
                ...
                >
                  <Plus size={14} /> Flag New Non-Conformity
                </button>
              </div>
            </div>
          ...
          <div className="modal" style={{ maxWidth: 500 }} onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <div className="form-group">
                <label className="form-label">Observation / Non-Conformity Description <span>*</span></label>
                <textarea
                  className="form-control"
                  rows={4}
              ...
              <div className="form-group">
                <label className="form-label">Upload NC Observation Sheet (PDF / Image)</label>
                <input
                  type="file"
                  className="form-control"
